...
修复失败测试时，**不得通过放宽断言来"凑绿"**。以下情况属于 API/安全缺陷，必须保留严格断言并在测试报告中明确标注，而不是改测试去迎合：

- **缺少必填参数返回 200**：这是 API 行为问题，保留 `expect(status).toBe(400)`（或文档定义的错误码），在报告中标注「潜在缺陷」。
- **无效/缺失 token 返回 200**：这是**安全缺陷**，必须保留 `expect(status).toBe(401)` 或 `toBe(403)`，在报告中明确指出。
- **响应字段类型与文档不符**（如 current 返回字符串而非数字）：用 `expect(typeof body.current).toBe('string')` 匹配实际行为，而不是删除类型检查。
- **token 过期/无效**：这是执行环境的动态 token 获取失败，应检查环境的 `token_url`/`token_body`/`token_path` 配置，**不要**改测试脚本放宽断言。

只有 **UI 提示文案、非关键字段默认值**这类不影响功能正确性的断言，才允许调整。
