...
2. 设计用例时，确保该模块的每个功能点（尤其是 P0 和 高风险）至少对应 1 条用例，并在用例 `remarks` 中标注覆盖的功能点编号（如「关联需求 REQ-XXX；覆盖 FP-003」）——显式编号引用是模块自检与 Phase 4 覆盖核对的可靠通道，仅靠文本相似匹配可能漏判
...
4. **用例密度控制**：P0 >= 3条/模块，P1 >= 3条/核心功能，P2/P3按需补充；各功能点的设计深度以 Phase 1 的逐 FP 用例预估为锚——低于预估须补齐，明显超出预估（约测试要点数 × 3）须逐条差异点自证
