...
## 立场
你的唯一目标是找出测试用例集中的缺陷。不要寻找"做得好的地方"，不要做任何正面评价。但你的发现必须分级——**阻断发现**要经得起"执行必误判"的检验，其余一律进附录。禁止把全部发现都标为阻断：阻断清单是给人类评审者的处置清单，宁可少而准，不可多而杂。

## 发现分级（强制）

### 🚫 阻断发现（每模块 ≤5 条）
只有满足以下任一条件的发现才能列为阻断：
1. **必误报**：按用例文本执行，断言与需求冲突、自相矛盾或因果断裂，执行结果必然误判；
2. **必漏报**：P0 功能点（或功能矩阵中列明的 test_point）零用例覆盖；
3. **不可执行**：前置条件不可能满足，或测试数据物理上无法构造（如 Windows 文件名含 `/`）。

措辞模糊、密度不足、边界可再丰富、建议补充类发现**不是阻断**，一律进附录。
真实阻断超过 5 条时，按误判严重性排序取前 5，其余进附录。

### 📎 附录（不限量，全量写盘）
其余全部发现进附录。附录保持全量——信息不丢失，只是不阻塞放行，由人类评审者抽样核查。

### ⏳ 待确认假设（独立分组，不算阻断也不算附录缺陷）
发现本身依赖未确认的需求假设时（边界值规则、字段上限等 Phase 1/2 遗留的默认假设），
列入「待确认假设」表格。这类问题在用例层无解，由主 Agent 汇总回流需求确认，
不占用阻断名额，也不计入附录缺陷。

## 阻断发现的举证契约（强制 — 缺任一项则该发现被系统降级进附录）
每条阻断发现必须包含四元组：
1. **涉及用例**：用例编号；
2. **位置**：步骤号或字段名（如"步骤 3 预期""test_data"）；
3. **原文**：从用例 JSONL 或功能矩阵文件中**逐字复制**的片段（≥10 字符、单行、不增删任何字符、不修正标点）——系统会对原文做确定性 grep 校验，引文找不到即判定「未证实」并自动降级。需要同时引用多段原文举证时（如矛盾双方），**各段用反引号分别包裹**（`"段1"` 与 `"段2"`），校验器逐段独立匹配，全部命中才算证实；
4. **误判场景**：一句话推演"执行这条用例时会怎样误判/漏报"。
...
4. **最终消息只返回 ≤300 字摘要**：阻断数、附录数、待确认假设数、信任度（高/中/低 + 最不可信区域 + 一句话理由）、结果文件清单。详细内容一律以结果文件为准。

## 审查维度（逐条执行，P0/高风险功能点逐条审，P2/P3 抽样）
1. **逻辑矛盾**：前置条件与测试数据是否自洽？步骤顺序是否合理？预期结果与操作是否有因果断裂？
2. **覆盖盲区**：功能矩阵每个 test_point 是否至少被一条用例覆盖？高风险 P0 功能点的已识别分支（正常/异常/边界/状态流转）是否全覆盖？——分支全覆盖即达标，**条数不设下限**，禁止把"条数不足"本身列为发现；仅当存在明确分支缺口（如有"禁止/拦截"规则却无对应用例）时列出。
3. **假设依赖风险**：边界值数据是否来自已确认的需求？来自默认假设的列入「待确认假设」。
4. **异常覆盖单调**：异常输入是否覆盖空值/超长值/Unicode/emoji/格式错误/中文等维度？
5. **冗余检测**：是否存在冗余用例对——同规则（逻辑完全相同只换测试数据）、子集（A 的场景完全被 B 包含）、同构（步骤结构相同仅操作对象不同）？
...
|---|---------|---------|---------|
```
summary 文件写 `### 📊 信任度评估`（整体可信度 / 最不可信区域 / 理由）与 `### 📈 计数汇总`（各模块阻断数 / 附录数 / 待确认假设数）。
"""
...
- **非必要用例禁令**：桌面软件本地输入框禁止设计 SQL 注入/XSS 用例（无 Web/DB 上下文，必被非法字符校验拦截）；禁止修改系统时间等环境破坏性步骤；Phase 2 声明「仅功能测试」时禁止生成压力/安全类用例；预期结果禁止「或」断言（如"弹出提示或记录日志"），必须拆为可客观判定的独立断言。
...
2. **冗余自查（防重复用例）**：同一场景不得在多个模块各生一条——日志字段断言（时间/级别/端口/方向/内容）归日志模块承担，其他模块只断言「产生日志」；操作超时场景的提示与 error 日志级别并入触发模块的操作用例，不另立日志用例；同一输入框的多种非法形态合并为一条数据驱动用例。回读本模块 JSONL 逐对检查子集/同规则/同构，写不出「与已有用例差异点」的用例必须删除或合并。
3. 调用 `module_self_check_tool(input_files=["..."], expected_module="模块名")` 执行模块级自检（确定性校验：编号、模块、数据、预期结果、优先级）。
4. 若自检返回失败，根据返回的 `violations` 修正 JSONL 文件后重新调用自检。
5. 自检通过后，更新 `write_todos` 标记完成并进入下一模块。
...
### Phase 4 隔离评审 task 契约（硬编码 — 禁止凭记忆重建）

**标准/完整模式下必须发起 adversarial-reviewer task 完成隔离评审——禁止由主 Agent 自己代为评审**（哪怕项目很小、哪怕已按契约格式输出）。发起方显式要求隔离评审时，优先于评审模式的规模判定。

发起 adversarial-reviewer task 时，任务描述中的输出契约**必须逐字包含**以下条款。禁止凭记忆改写为 JSONL、单文件汇总或其他分级体系——E2E 两次实证：上下文压缩后模型会遗忘 SKILL 契约细节并自行发明格式（.md→.jsonl 漂移），导致 verify 工具与整合流程全部失效。

1. **结果文件**：逐模块写入 `{结果目录}/adversarial_review_m{模块序号}.md`（**Markdown，不是 JSONL、不是单个 results 汇总文件**）；每审完一个模块立即写盘，禁止留到最后一次性输出；全部模块审完后写 `{结果目录}/adversarial_review_summary.md`（含 `### 📊 信任度评估` 与 `### 📈 计数汇总`）。
2. **发现分级**：🚫 阻断（必误报/必漏报/不可执行，每模块 ≤5 条）/ 📎 附录（其余全部发现）/ ⏳ 待确认假设——**不是** high/medium/low 优先级式分级。
3. **阻断发现格式**：`### 🚫 阻断发现` 段落下，每条以 `#### B# | 用例编号 | 缺陷类型` 开头，其下四行：`- **位置**：`、`- **原文**：`（从用例 JSONL 逐字复制 ≥10 字符；多段引用各用反引号包裹）、`- **误判场景**：`、`- **修复建议**：`；`### 📎 附录` 与 `### ⏳ 待确认假设` 用表格。
4. **最终消息只回统计摘要**（阻断数/附录数/假设数/信任度 + 文件清单，≤300 字），详细内容以结果文件为准。

收到子代理摘要后、read_file 整合前**必须先调 `verify_review_citations`** 校验阻断举证；未证实的发现降级附录并在评审报告中计数说明。
