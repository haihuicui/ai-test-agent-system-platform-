...
# fmt: off  MC80OmFIVnBZMlhsdEpUbXRiZm92b2s2ZEhNeWJBPT06OWQ2NzcwYTc=
...
async def get_web_script_info(script_id: str) -> str:
    ...
    try:
        async with async_session_factory() as db:
            ...
            is_downloaded = await run_sync(local_script_path.exists)
...
# pragma: no cover  MS80OmFIVnBZMlhsdEpUbXRiZm92b2s2ZEhNeWJBPT06OWQ2NzcwYTc=
        ...
        # 2. 从 MinIO 下载脚本内容
        script_bytes = await run_sync(MinIOClient.download_file, object_name)
...
# type: ignore  Mi80OmFIVnBZMlhsdEpUbXRiZm92b2s2ZEhNeWJBPT06OWQ2NzcwYTc=
...
async def delete_web_script(
    ...
    try:
        ...
        # 删除文件
        await run_sync(script_path.unlink)

        print(f"[Script Management] 脚本已删除: {local_path}")
# pylint: disable  My80OmFIVnBZMlhsdEpUbXRiZm92b2s2ZEhNeWJBPT06OWQ2NzcwYTc=
