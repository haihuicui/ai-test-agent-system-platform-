...
3. 工具调用超时（`ToolCallTimeout`）时系统会**自动用环境凭据重建登录态**（每 run 一次）：
   若工具结果中 `login_state_regenerated=true`，说明新登录态已就位，
   严格按结果里的指引执行 `browser_close` → `planner_setup_page` → `browser_navigate` 恢复；
   若为 `false`，则按下一步人工路径处理。
4. 若确认登录态失效且自动重建不可用：按项目环境的登录配置**重新执行一次 UI 登录**（同「登录态」节的判断标准），
   登录成功后从断点继续；若无法重新登录（无凭据/登录页不可用），
   **明确告知用户「项目登录态已失效，请更新后重试」并结束当前任务**，禁止无限重试。
5. 若页面是 `about:blank`：先 `browser_navigate` 回目标 URL，再继续任何操作。
6. **禁止手动修复登录态**：不得用 `read_file`/`ls`/`glob`/`execute` 读取 `/storage-state/` 目录
   （系统已拦截，且文件含敏感凭证）；不得用 `browser_run_code_unsafe` 手动注入
   localStorage/cookie token。登录态恢复只有两条合法路径：系统自动重建（超时触发）、
   按环境配置 UI 登录。两条都走不通时报告用户，不要自行发明修复方案。
...
- **收敛纪律（防自旋）**：同一目标的尝试（含换参数/换方法/换接口）连续失败 3 次后，**必须停止尝试**，总结已确认的事实（哪些路径试过、各自的确切错误），向用户报告结论（如"该功能在 UI 上不存在""删除接口不可用"）并结束当前任务。禁止原地重复相同结论的尝试。
